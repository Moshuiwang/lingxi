...
> 状态：数据面 A（组织快照）已由生产职责接管；数据面 B（多维表格关联）仍是历史受控验证记录，正式业务入口尚未接入。
> 最后验证：2026-08-05，`biai-stage` / `Bot-Test` / 测试 PostgreSQL。
> **2026-08-23 #146 清退更新**：入口脚本 `scripts/sync_feishu_org_snapshot.py` 已删除。它写入的是测试库（`LINGXI_TEST_POSTGRES_DSN`），且早在 2026-08-19 就已确认因所依赖的 `feishu_user_refresh_token` 测试表不在当前 alembic 架构里而**无法运行**（见下方 2.1 节原文）；生产侧的组织快照现由 [#250](https://github.com/Moshuiwang/lingxi/issues/250) 的 `OrgSnapshotSyncDuty`（`apps/scheduler/org_snapshot_sync.py`）每 UTC 日写真实 `feishu_org_*` 快照表接管，不再依赖本文档的测试库路径。以下 2.1、4.1 两节按原样保留作为**历史记录**，说明当初那一轮受控验证是怎么做的、拿到了什么；不是可复用的操作步骤，也不代表当前的生产事实。
...
入口脚本曾是 `scripts/sync_feishu_org_snapshot.py`（已随 #146 清退删除，见文首更新说明）。它在测试环境中执行两条身份路径，并在完整性校验通过后，把一轮结果写入测试库：
...
>
> **本节以下两处结论已被 [#250](https://github.com/Moshuiwang/lingxi/issues/250) 取代（2026-08-23 补记）**：`adapters/postgres_identity.py` 的 `PostgresOrgSnapshotStore` 当时确实零调用方，`biai-stage` 上四张快照表当时确实全空；#250 交付的 `OrgSnapshotSyncDuty`（`apps/scheduler/org_snapshot_sync.py`）已经是这里描述的「正规做法」的实现——`lingxi-scheduler` 的一条定时职责，走 `commit_batch`，复用凭据轮换职责进程内的派生令牌持有者，每 UTC 日写真实的（不是测试库的）四张 `feishu_org_*` 快照表。以下两段按原样保留作为该职责落地前的历史判断记录。
>
...
成员快照的标准化字段包括（**以下为 005 测试库阶段字段**，是当初那一轮受控验证读取到的原始响应形状，不是当前正式表列）：
...
**正式表（`20260806_baseline_006_012.py`，`feishu_org_member_snapshot`）实际列**与上面不同：`tenant_key`、`member_key`、`open_id`、`user_id`、`union_id`、`display_name`、`display_name_locale`、`department_names`（JSONB）。正式化相对 005 的改动：删掉 `status` 列（在职过滤已改为实时回读，不落陈旧状态）；删掉 `app_records`/`user_records`/`detail_record`/`avatar` 等原始响应列（正式表只存身份关联必需字段）；`open_id`/`user_id`/`union_id` 改为 `NOT NULL` 且非空白校验；`name`/`i18n_name` 合并为 `display_name`/`display_name_locale`（姓名不假定语言，`locale` 仅展示辅助）；`parent_department_ids` 由 `department_names`（JSONB 数组）取代。

对应的表（表名在测试库与生产库共用同一份 `20260806_baseline_006_012.py` 迁移，2.1 节其余原文描述的是当初测试库阶段的读取方式，非表结构本身）：
...
读取入口曾是 `scripts/read_feishu_bitable_association.py`（**已随 #146 清退批于 2026-08-23 移除**：其依赖的 `sync_feishu_org_snapshot.py` 与 `migrations/testing/005` 列形状均已清退，脚本自 2026-08-19 起本已不可运行；历史用法仅供追溯）。它使用已加密保存的 User 续期凭据换取短期 `user_access_token`，然后只读调用多维表格 API：
...
（历史命令，脚本已随 #146 清退批于 2026-08-23 移除）PYTHONPATH=src python3 scripts/read_feishu_bitable_association.py
```
