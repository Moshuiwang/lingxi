...
>
> 现行规则：产品负责人于 2026-07-26 确认 Lingxi 数据库是用户权限的唯一权威来源，飞书多维表格只是数据库向 MCP 发布权限的下游通道；首次自动准入按数据库中的当前有效权限记录处理。本文第 3 节及其中将银河记录直接作为授权依据的历史方案不再作为现行身份流程；请以[产品合同](../产品合同与外部边界.md)及[自动准入与数据库权限发布边界决策](../决策记录/2026-07-26-自动准入与数据库权限发布边界.md)为准。
