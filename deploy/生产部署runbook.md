...
### 2.3.7 `v2.3.1` → `v2.4.3` 升级要点（Trace [#754](https://github.com/Moshuiwang/lingxi/issues/754) 补入）

本节汇总本批（三个此前只在预发验证过的版本 `2.4.0` / `2.4.1` / `2.4.2` 一次性发布到生产）相对「2.3 迁移」正文的差异点；命令形态仍以 2.3 / 2.3.5 / 2.3.6 为准，这里只补生产专属的判据与顺序。

① **迁移链**：现网迁移头是 `0090_delivery_retry_backoff`；本批链头是 `0096_plpgsql_search_path`。`run --rm migrate` 一次把 `0091`–`0096` 全部前滚，中间不停顿、不分批执行。
...
- **（2026-09-02 更新：撤销条件已满足，手工过渡已停止。）** 首发当天 `lingxi-prod` 没有备份点（产品负责人 R8：首发豁免恢复演练），过渡方案是由部署用户每日一次 `pg_dump -Fc` 到自己的目录（0600）。**Supabase 托管每日备份/PITR 已于 2026-09-02 开启，同一天手工每日 dump 已停止**（[Issue #693](https://github.com/Moshuiwang/lingxi/issues/693) 分诊回源 Trace #544 / #592 任务表）——按上面写的撤销条件，手工过渡到此结束，不再执行，`lingxi-prod` 的备份点现在是托管备份。**升级前仍要新取一份 `pg_dump -Fc` 作为本次升级独立的恢复点**（合同 S-3-2，见「二、首次部署步骤」对应生产执行卡步骤），这是每次升级各自的动作，不依赖已撤销的每日手工过渡，也不能用托管备份的既有快照代替。`scripts/ops/backup_restore_drill.sh` 是演练脚本、不是备份计划，不能顶用。
- **演练脚本主机客户端版本前提（2026-09-11，Trace #754 W1 S-1-3）**：预发主机安装的 `pg_dump`/`pg_restore`/`pg_isready`/`psql` 是 16.15，直接连 Supabase 服务端（17.6）会报 `server version mismatch` 并以非零退出（编排者 2026-09-11 在预发主机实测）；生产主机客户端是 17.10，同一大版本本可直接连接，但 `scripts/ops/backup_restore_drill.sh` 对源库的全部客户端调用统一经一次性 `postgres:17` 容器执行、不按主机版本分叉——预发与生产因此走同一条执行路径。脚本现状见「八、恢复演练要求」。
...
>
> **2026-09-04 实录**：本节已在生产真实执行一次（金丝雀单人：dry-run → 单字 → `--apply` → 激活，随后按产品负责人裁定撤销——**仓库没有「预开通整组撤销并删档」的运维入口**，撤销是在一个事务里按精确行删除六张表并逐表断言行数、不符即整体回滚，再删正式表记录并按邮箱回读 0；生产删除必须带这道断言保护，首次因 id 截断断言不符即整体回滚零写入），此前同日先在 stage 用真实名单演练三次才走通——三次里前两次分别被 §12.0 的内测白名单闸与 §12.2 的令牌间隔规则挡住、零写入。**新入口复用旧链的改动，stage 真实名单演练必须早于生产窗口一天**，不能放在窗口当刻。
...
探针、单次探针超时 20 秒，见 `src/lingxi/core/permission/mcp_readiness_base.py`），加上身份
...
> **本节回答的问题**：生产已经在跑、名单要再加几个人时，从哪一步开始动、动完为什么还不能马上跑。
> §十二 写的是升级窗口内跟着 `up -d` 一起做的批量预开通；**不在升级窗口内单独加人**走的是本节，
> 三处差别都能让整份名单零写入，而且失败形态都不像失败（退出码 0、"跳过"、"拒绝"）。

### 14.1 五步顺序（不可调换）

| 步 | 动作 | 不做会怎样 |
| --- | --- | --- |
| 1 | 把新增人员的 Bot-Prod `open_id` 追加进 `.env.prod.scheduler` 与 `.env.prod.gateway` 的 `LINGXI_INNERTEST_ROSTER_OPEN_IDS`（两文件同值） | 名单外的人**连建档都进不去**，逐人报 `innertest_roster_rejected`、零写入 |
| 2 | `--profile mvp up -d` **重建** scheduler（不是 `restart`） | `env_file` 的改动不会进到已在跑的容器里，第 1 步等于没做 |
| 3 | 重建后**等约 5 分钟**再跑脚本 | 整份名单失败关闭、跳过、零写入，报 `employment_read_failed_AccessTokenUnavailableError` |
| 4 | 名单 CSV 的职位列写**随包角色映射里真实存在的键** | 整份名单被拒、零写入 |
| 5 | dry-run 核对条数 → 产品负责人放行 → `--apply`（姿势见 §12.2） | —— |

### 14.2 内测名单是必经闸，它在开通链的最前面

`LINGXI_INNERTEST_ROSTER_OPEN_IDS` 挡在 `core/identity/onboarding_runner._gate_innertest`，
比身份定位、建档、权限翻译都早。**它不是"某项功能的开关"，是"这个人能不能进入产品"的总闸**：
名单外的人一步都走不动。变量的两份载体与写法见 §11.4，名单到手后的常规追加见 §12.0。

2026-09-07 实录：为了给宋威、陆啸两位开通，该变量从 **34 条加到 36 条**。

### 14.3 重建 scheduler 之后必须等约 5 分钟——这是正确行为，不是缺陷

`env_file` 的值在容器创建时固化，**`docker compose restart` 不会重读**，必须 `up -d` 让 compose
重建容器。而容器一重建，在职状态实时回读要用的**独占派生令牌持有者是空的**：预开通脚本是
scheduler 之外的第二个进程，它自己去领令牌时撞上 §12.2 那条「最小续期间隔 5 分钟」，于是名单
里每个人都报 `employment_read_failed_AccessTokenUnavailableError`，整条链**失败关闭、跳过、
零写入、退出码 0**。

2026-09-07 实录：09:27:11 重建 scheduler，09:28:40 起脚本 → 全员跳过；等满 6 分钟后原样重跑 →
成功。**不知道这一条的人会把它读成"开通功能坏了"**，然后去查代码；正确的动作是等满间隔重跑。

判读要点：日志里带 `AccessTokenUnavailable` 且退出码为 0、写入计数为 0 的，一律先看重建时刻。

### 14.4 职位必须精确命中随包角色映射的键

名单 CSV 的 `position` 列不是自由文本，它要在 `src/lingxi/config/galaxy_role_function_map.toml`
里逐字命中一个键。写「销售」这种职能名会让**整份名单被拒、零写入**——真实的键长
`A销售（所有）`、`A销售_中文包` 这样。写名单之前先从该文件里取键，不要凭职位描述臆造。

**指标条数由「职能」决定，不由公司范围决定**。角色映射先把角色键翻成职能（`A销售（所有）` →
「销售」、`A运营_CEO` → 「运营」），再由公司＋职能映射给出指标：

| 职能 | 指标数 | 指标（中文名） | 「全部公司」（43 家）的预授权条数 |
| --- | --- | --- | --- |
| 销售 | 3 | 新增用户数、汇率、税率 | 43 × 3 = **129** |
| CEO / 运营 | 9 | 全部九个 | 43 × 9 = **387** |

dry-run 打印的条数与这张表对不上，说明职位列命中的键跟预期不是同一个——**先查名单，不要直接
`--apply`**。

### 14.5 两份映射表的载体差别（决定"改它要不要发版"）

| 映射 | 文件 | 能否外置 | 改它要做什么 |
| --- | --- | --- | --- |
| 角色 → 职能 | `galaxy_role_function_map.toml` | **不能**，没有外置口 | 改代码 → 出镜像 → 发一版 |
| 公司＋职能 → 指标 | `company_function_metric_map.toml` | **能**：`LINGXI_COMPANY_FUNCTION_METRIC_MAP_PATH` 指向 `runtime-config` 下的覆盖文件 | 放文件 + 配变量；**生产当前未配**，首次配变量必须 `up -d`（同 14.3） |

外置那一份要配就 scheduler 与 gateway 两侧同值同在，要么两边都不配（见 `docs/当前能力.md`
「权限与授权」一节）。指标的**中文名**在第三份表 `admin_metric_alias_map.toml` 里，同样随镜像
发布、**没有外置口**——2026-09-07 改六个指标中文名就是为此单独发了一版 `v2.3.1`。

### 14.6 加完人之后：主动发欢迎卡这一步有平台前提

预开通成功**不等于这个人能收到主动消息**。机器人无法向从未与它对话过的跨组织用户发起单聊
（飞书 `230013`），完整证据与替代触达办法见
[飞书机器人主动私聊的跨组织前提](../docs/参考证据/飞书机器人主动私聊的跨组织前提.md)。
排窗口时要把这一条算进去：§十三 的 `--apply` 对这类人会逐人 `failed`，不是配置能修的。

### 14.7 收件人必须用白名单圈定，不得用反向排除推导

面向真实用户的批量发送，**收件人集合只能由写死的白名单给出**；不得用「已激活 ＋ 有过入站消息 ＋
没有送达记录」这类**反向排除**条件推导出收件人。

2026-09-07 实录：一个补发守望脚本按上面那组反向条件筛人，产品负责人本人三条全中（他一直在与
机器人对话、名下只有预检记录），于是在 10:01:00Z 收到一张本不该发给他的正式欢迎卡。已核实只误发
