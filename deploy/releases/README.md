...
预发布附件 `release-manifest.json` 保存源码、四镜像摘要、迁移头和构建记录。上传中断可继续，既有不同内容不覆盖。正式版本标签和附件发布后不得改写；有变更建立新候选。
...
在 GitHub Actions 选择 `Release Promotion`、分支 main、填候选标签。工作流先只读核对，再将同一组镜像发布为 `vX.Y.Z`，全程不构建、不部署。候选没通过、记录缺失或不一致均拒绝。
...
测试环境将 environment 改为 stage，可选择 rc。生产拒绝 rc，且必须回读正式版本所引用的候选、成功构建与 main 中的验收记录。该命令不修改现有配置、不接触数据库、不运行 Compose。#566 后续部署脚本应复用此入口；使用旧命令直接指定镜像的人工操作无法由本脚本拦截，不得声称已存在覆盖所有生产入口的强制控制。
