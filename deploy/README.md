...
验收前要**配齐什么**、缺一项会死在哪里、配完怎么自检，见[验收前部署配置清单](验收前部署配置清单.md)——本文件负责"怎么跑起来"，那份文件负责"跑起来之前要凑齐哪些东西"。生产首次部署的分步骤、镜像 digest 固定、单实例纪律、首发不灰度、回滚判据与观察期见[生产部署 runbook](生产部署runbook.md)；基础设施监控与告警现状见[监控告警](监控告警.md)；容器日志留存现状见[日志留存](日志留存.md)；把一个预发测试账号清回「从未开通」以重走开通旅程（预发专用、生产禁用）见[预发用户开通记录复位](预发用户开通记录复位.md)。
...
**读日志时按摘要里的原文认**（`scripts/ci/push_image.py` 的现行输出）。**绿色作业有三种结局，不是两种**：

- **推上去了**：摘要写「**镜像已推送 GHCR**」，并回读 digest。GHCR 上那一份就是权威制品。
- **本来就在，没必要推**：摘要写「**镜像已在 GHCR，来源相同，跳过推送**」（重跑或补推时常见）。**这也是成功**——制品在 GHCR 上。
...
**只按前两种去认会读错**：重跑时看不到「已推送」那句，容易误判成「镜像没上 GHCR」。
...
- **规则**：[SemVer 2.0.0](https://semver.org/lang/zh-CN/)（`主.次.补丁`）。破坏用户承诺或外部边界升主版本，新增能力升次版本，修复升补丁版本。工程基线的重大变化（如代码规范与门禁体系落地）也按次版本处理（产品负责人 2026-09-05 裁定，首例 `2.1.0`）。本仓库此前无对外版本号，首版定 `2.0.0`。
...
1. **合并点必须有镜像证明，且这一条判红而不是判文档自觉。** `epic/**` 的合并点
   ...
   真的存在，不能只写在文档里。
   **一处容易读错的地方**（独立审查实测后补记）：这个红**本来就会红**——
   `is_epic_head` 对 `trace/**` 永远是 0，所以没有镜像证明的 `trace/**` 候选原本
   就会落到最后那条「三项必须全 success」上判红。本条**新增的不是红本身，而是一个
   能诊断的红**：把「Epic 候选未冻结」这句读不出下一步的失败信息，换成直接写明
   要补什么、怎么补。安全性来自那条兜底，可用性来自这条信息，两者不要混为一谈。
