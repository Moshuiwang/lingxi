...
1. **原发送路径**——刚跑完四步流程的那次调用（``apps/gateway/document_delivery.py
   ...
   路径能拿到 :class:`~lingxi.adapters.feishu_docx_delivery.WriteBodyOutcome`。
...
3. **检查点恢复路径**——写正文之后、落 ``succeeded`` 之前进程崩溃/被
   ...
   判定"正文已经写过"直接跳过写正文步，**因此永远不会再调用一次 ``write_body``、
   也就永远拿不到那个内存信号**。
...
不与后续"授权/读回/落终态"共享事务）。``NULL`` = 没有降级（转换开关关时的段落
路径、或转换路径原样成功——两者都是这套部署本该给出的排版，不是降级）；非
``NULL`` = 正文改用纯文本段落路径写入，取值是原因码（当前唯一取值
