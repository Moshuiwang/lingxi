...
> **2026-08-17 更新（S-C-02，产品负责人裁定）**：上面这句话的后半段已被取代，本表结构
> 不变。现行语义是「**新建**发布行必须携带 Lingxi 签发的 ``token_cipher``；**更新**既有
> 行时它既不被清空也不被覆盖」——没有令牌的行对问数 MCP 毫无意义（MCP 逐行解密后与请求
> Bearer 明文等值匹配）。因此 ``payload`` 在有令牌时是**七个键**（多一个 ``token_cipher``），
> 存的是**密文**；令牌**明文**与主密钥仍然一步都不进 payload、日志与告警。正文见
> ``src/lingxi/core/permission/publish_row.py`` 的模块文档与迁移 ``0065``；同一口径的
> 长期正文在[数据库设计「四、开通与发布」](../../../docs/技术设计/数据库设计.md#四开通与发布)
> 第 1 条与[验收矩阵](../../../docs/技术设计/验收矩阵-权限与银河.md)的 ``V-权限-11``（数据库设计
> 那一处的旧措辞于 2026-08-18 订正 —— Epic C 冻结缺陷 F3；验收矩阵那一处早在
> 2026-08-17 裁定当日就已改到位）。
