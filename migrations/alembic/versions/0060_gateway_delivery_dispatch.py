...
3. **``dispatch_reserved_kind``**：外发前预留位（P3-6：#151 审核明确要求 #152 在没有
   ...
   调用前提交这一列非空、调用有了明确结果后立即清空；**只有服务端明确拒绝才清空**
   （独立审核 B-1，2026-08-14 修正；见下）。
   **进程崩溃、或外发调用抛出结果不明的网络类异常（``requests`` 的超时/连接类异常，
   全部是内置 ``OSError`` 的子类——``lark-oapi`` 的 transport 就是 ``requests``）
   发生在提交与清空之间时，重启后这一列仍非空，消费循环必须把该任务视为
   ``uncertain``、停止对它的自动处理并告警，不得凭空猜测上一次调用是否成功后自动重发**
   （Issue 状态合同第 6 条：不得自动重发造成重复结果）。恢复需要人工核对对应外发是否
   实际已经成功（按 ``card_id``/``message_id`` 回读，或按话题+内容核对）——已送达则
   把回读到的标识通过 ``record_delivery_progress`` 写回，不重放外部调用；确认未送达
   才清空预留位允许重试（独立审核 B-2 补全，完整两分支写在
   ``src/lingxi/apps/gateway/delivery.py`` 模块头注释）。
