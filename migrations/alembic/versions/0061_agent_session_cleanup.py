...
可被调用的**常驻** Worker 进程去执行物理文件删除，而常驵 Worker 由本 Story 第一次
建立。
...
CREATE TABLE agent_session_cleanup (
    ...
    reason             TEXT        NOT NULL
        CHECK (reason IN ('new_command', 'idle_timeout', 'user_cleared')),
    queued_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    claimed_at         TIMESTAMPTZ,
    done_at            TIMESTAMPTZ
);

-- 同一个 agent_session_id 只需要排队一次：触发点在插入前用 NOT EXISTS 去重
-- （见 postgres_conversation.py），这条唯一索引是数据库层面的第二道保险，
-- 防止并发触发（例如空闲到点扫描恰好与 /new 撞在同一时刻）产生重复的清理请求。
