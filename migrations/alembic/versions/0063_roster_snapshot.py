...
**保留期的已知留白（本 revision 不解决，登记在案）**：本表承载可识别人员资料，但
**没有 ``expires_at``、没有到期触发器、也不在 ``0054`` 的受限清理函数覆盖范围内**。
理由是「保留上一份有效快照直到下一次成功读取」是产品负责人的明确决定，与「九十天
到期即删」在源头长期读不到时会直接冲突——删掉最后一份快照等于让比对失去基线。正常
运行下每天成功读取一次即整体替换，不积累历史版本，因此这里不是「越攒越多」的形态；
但「源头连续九十天读不到」这一情形下该保留还是该清理，属于产品保留边界的选择，需要
产品负责人裁定后由后续 Story 落地（与 ``0059`` 登记 ``task_delivery_event`` 行删除
留白的处理方式相同）。
