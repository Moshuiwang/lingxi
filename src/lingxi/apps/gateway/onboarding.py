...
def assert_gateway_onboarding_is_inert(*runners: Any) -> None:
    ...
    管线在**长连接事件线程**里同步调用开通入口。真实编排单次耗时可达分钟级（合同允许
    权限同步等到十五分钟），接上去的后果是 gateway 十五分钟收不到任何消息——而现场表现
    只是「机器人不理人」，没有任何一条日志会说明原因。

    Args:
        *runners: 装配层回报的、**实际被采用**的开通实现。**至少要一个**：一个都没报告
            说明回报钩子根本没触发，断言会退化成永远成立的空话。
...
class _RecordingOnboarding:
    ...
    因此本类返回"已接手"：管线对这个状态**刻意不记账**，账本留给真正跑完的那一方，
    中途崩溃的链才能被重新认领。这**不是失败关闭桩**——桩会让每个未开通用户当场看到
    内部故障码，而这里说的是真话。
    """
