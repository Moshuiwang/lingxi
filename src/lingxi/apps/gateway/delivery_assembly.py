...
class RejectingCards:
    ...
    只在显式设置故障注入环境变量时才会被装配；未设置（默认）时装配路径与本类
    加入之前逐字节一致。实现刻意最简单——只让被选中的那一步失败，未选中的步骤直通
    真实 transport，不建"命中一次之后整个任务转失败"的状态机。

    四个取值的实测语义：``create`` 与 ``all`` 在正常单轮场景下等价（建卡先被拒、
    任务立即整体降级，后两步没有机会被调用）；覆盖"建卡成功之后终态更新失败"这条
    降级路径的是 ``update``；``close`` 单独命中**不产生降级**——终态已经正确显示，
    再补一条文本就成了重复交付，因此它是"关闭失败不得产生第二条文本终态"的验收入口。
    """
