...
def combined_heartbeat(
    ...
    两件事共用同一个触发时机——都是「这条循环这一轮还活着」的证据——但服务不同的消费者：
    告警状态机供跨进程重启也能观察到的阈值与去重判定，活性文件供同容器内的健康检查命令
    判断「主循环是否还在跳动」。

    ``watchdog`` 是搭在同一个时机上的第三件事——这条循环还活着的时候，顺手确认另一条也
    还活着。放在心跳之后调用，看门狗异常不连累真正的心跳工作。
    """
...
def run_delivery_loop(
    ...
    *,
    ...
    消费者自己已经把单轮异常隔离掉，不会因为一次瞬时数据库错误退出；但「线程没了」这件
    事不能只由那一层保证——解释器级错误、或将来某次改动把隔离改漏，都会让它无声消失。

    ``on_dead`` 覆盖两条路径：异常退出，以及「没有收到停机信号却正常返回」。停机信号已
    置位时的正常返回是**预期**退出，不上报——那是优雅停机，不是故障。
    """
    try:
