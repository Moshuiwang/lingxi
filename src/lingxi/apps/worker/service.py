...
class WorkerService:
    ...
    async def _run_rolling_claim_loop(self, terminal_tasks: list[TerminalTask]) -> bool:
        ...
        旧的"整批领、整批等"实现里并发上限实际只是一次领取的批大小，一个长任务会让批内
        与批后到达的所有任务一起干等到它收口。每次等待都用轮询间隔兜底，因此即使一个都
        没终态（长任务仍在跑、空槽从一开始就没填满）也会按既有节奏重新尝试领取。

        停机语义不弱化：一旦置位就不再领取新任务，只让已在途的自然收尾。领取前的多轮
        让出只需要在**进入这个循环之前**做一次——循环内后续每次领取之前都已经跨过一次
        真正的等待，事件循环有充分机会把信号投递链路跑完。

        巡检节拍也要自己维持：入口只在进入循环前巡检一次，而持续有任务可领时这个循环要
        等在途集合清空才返回。旧实现里巡检天然随批次间隙被反复调用，滚动并发把间隙消灭
        之后，排队超时收口、版本不可用、心跳超时回收、投递过期、会话清理会在供给不断时
        整体停摆到进程下一次真正无事可做为止。复用轮询间隔当节拍，不新开配置项。
        """
