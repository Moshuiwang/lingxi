...
class TurnOutcome:
    ...
    def stopped_by_interrupt(self) -> bool:
        ...
        只认执行层给出的真中断——它是观测到本地停止信号已置位、真正调用过中断之后才
        抛出的，是这条链路上唯一带因果的信号。队列侧的"有人请求过停止"只说明某一刻有
        人按过，不说明这一轮的结果由它决定：让一次并发到达的停止压过所有已经发生的
        事实，会把真实失败原因改写成"已停止"，也会把一个已经产出结果的成功降级掉，
        连同只有成功分支才写的会话标识一起丢失——那是"重启与重试不得造成用户结果
        丢失"这条红线。

        尤其不能用"没有失败码"反推成停止：收口失败本身就是失败事实，没有失败码只是
        没人给它起名字。也不能把执行层自报的"已中止"当成停止的别名——它与本地停止
        信号之间没有任何因果绑定。
        """
