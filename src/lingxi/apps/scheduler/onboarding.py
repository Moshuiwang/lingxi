...
class OnboardingExecutor:
    """开通链的专属线程池。**不与长连接线程、投递线程共用任何一条**：真实编排
    单次可达分钟级（权限同步允许等到十五分钟），跑在长连接线程上会让 gateway
    十五分钟收不到任何消息，跑在投递线程上会让十五分钟一条投递都发不出去。
    **队列有界**，满了就拒绝而不是无限堆积：一次外部故障导致所有链都卡在
    十五分钟等待时，继续排队只会让排在后面的用户更晚收到一条早已过时的结论；
    拒绝会让调用方返回明确的内部故障终态。线程是 ``daemon``：停机预算耗尽时
    进程不会被一条还在等外部响应的链焊住。
    """
...
def build_stock_token_source(
    ...
    *,
    ...
    """装配存量令牌只读源；坐标、MCP 令牌主密钥或令牌供给缺一即不装配，返回
    ``None``——开通链因此原样走原签发路径，与改动前逐字节一致。

    坐标**都**未配置是唯一保持零信号的分支：该能力默认关闭，本函数只返回
    ``None``、调用方原样往下走。其余返回 ``None`` 的分支都是**半开的错误
    配置**，各留恰一条审计（只报症状分类，不回显任何配置值）：坐标只配
    一半、坐标齐但主密钥没接线、坐标与主密钥都齐但调用方没有交出令牌供给。
    复用权限发布表的应用身份令牌供给，不新增任何凭据材料。
    """
...
class _OnboardingRunnerInputs:
    """:func:`_build_onboarding_runner` 收到的可选输入，打包成一个值对象只是为了
    让该函数的参数个数落在 ``PLR0913`` 的上限内——字段本身仍在各自的构造点被
    直接使用，不是无意义的透传。
    """
...
def _build_onboarding_runner(
    ...
    *,
    ...
    return AutoOnboardingRunner(**kwargs)
