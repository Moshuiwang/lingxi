...
class OrgSnapshotSyncDuty:
    ...
    def run_once(self) -> str | None:
        """调用方（`SchedulerLoop`）每个 tick 调用的入口。

        只做本地、无 I/O 的判断决定"该不该起一轮"；真正的一轮（`_run_round`）
        整体派进后台线程，本方法只同步等一个很短的上限就把控制权交还调用方。
        正常情况下（既有测试的假读取都是微秒级）行为与同步调用完全一致；
        真正跑满整轮预算的那一轮，这里提前返回 ``None``——不是"这一轮失败
        了"，只是"调用方不再等"，本轮真正的结果由后台线程自己收口。本方法
...
def _stop_aware_sleep(stop: threading.Event) -> Callable[[float], None]:
    """把 `stop.wait` 包一层，让「停机置位后的等待」变成「中止」而不是「假装等过了、照样放行」。
