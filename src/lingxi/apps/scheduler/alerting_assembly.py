...
``AlertingDuty`` 状态机本身在 :mod:`lingxi.core.alerting`：gateway / worker /
scheduler 三个进程各自装配同型的一份，互不共享状态；这里只有 scheduler 专属的
装配细节——没配管理群时的仅日志出口、以及把 ``AlertManager`` 心跳与活性文件戳
合成一个回调。
"""
...
def build_alerting_duty(config: SchedulerConfig, *, audit: AuditSink | None = None) -> AlertingDuty:
    """装配 scheduler 自己的告警状态机。

    ``main()`` 在此之前从未实例化 ``AlertingDuty``；这是补上 scheduler 这一份的
