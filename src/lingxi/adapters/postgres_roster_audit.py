...
「只读」是产品约束，不是编码习惯：`employee_no` 是匹配银河权限记录的主键，把
花名册当前值静默写回存档会在下一次匹配时改变匹配结果，而日报恰恰是为了让人
判断这次变化该不该被接受（`V-花名册-14`）；存档三字段是比对基线本身，写回
等于每天把基线对齐成花名册现值，漂移不会消失、只会再也报不出来。

比对集的两条过滤都在 SQL 里，不在 Python 里（`V-花名册-10`、`V-花名册-11`）：
只比对已开通用户（`provisioning_state = 'active'`，其余状态尚未完成开通，出现
在管理群里是数据范围外泄）；排除 ``deleting``/``deleted``（删除编排会清空姓名
等字段，不排除会让每个被删除用户都稳定报一条「姓名变化」）。飞书在职状态与
花名册账号状态按需回读、不落库，本模块因此不 SELECT、更不写入任何状态快照列
（`V-花名册-15`）。
"""
...
class PostgresRosterBaselineReader:
    ...
    def load_active_baseline(self) -> tuple[ArchivedIdentity, ...]:
        ...
        # 只记条数（`V-花名册-33`：审计与日志不含花名册字段值）——不是日报的展示
        # 口径，日志会流向排障、CI 输出与工单，不在受控管理群那个范围里。
