...
class _TaskQueueBase:
    ...
    def _run_polling_operation(self, operation: Callable[[Any], _T]) -> _T:
        ...
        打开复用后，失败语义按这次调用一开始拿到的连接来源区分（**失败语义与
        改动前不再完全一致**——差别只在下面这一条"复用连接首次失败"分支，其余
        分支保持原样）：

        - **复用命中**（``self._pooled_connection`` 里已经缓存着一条可用连接）：
          ``operation`` 第一次执行失败，视为"服务端在两次轮询之间悄悄掐断了这条
          连接、而数据库本身健康"这一种具体场景（例如空闲连接超时）——丢弃这条
          连接、重新建立一条、把 ``operation`` **完整重新执行一次**。这一次重试
          （无论是重建连接本身失败，还是重试的 ``operation`` 又失败）都原样向上
          抛出，不再重试第二次、不吞任何最终失败。
        - **新建**（缓存为空或已关闭，本次调用现建的连接）：失败直接原样上抛，
          不重试——一条刚建好就用不了的连接大概率是数据库真的不可达，重试没有
          意义，也要避免"一次调用悄悄建两条新连接才放弃"的行为膨胀。

        调用方因此看到的失败语义：新建连接失败与旧行为完全一致；复用连接失败
        从"必然让这一轮轮询失败一次"改善为"服务端悄悄断开这一种具体场景对调用
        方大多透明"，其余失败路径（重试仍失败、新建连接失败）保持如实上抛，不
        产生任何新的静默吞异常行为。
        """
        ...
        try:
            ...
            if not reused:
                raise
            # 复用连接首次失败：重建一条连接，把同一次操作完整重试一次。重建
            # 本身失败（下面这行 `connect()`）或重试的 `operation` 再次失败，
            # 都不再兜底，原样向上抛出。
            rebuilt = connect(self._dsn, timeouts=self._timeouts)
            self._pooled_connection = rebuilt
            try:
                result = operation(rebuilt)
                rebuilt.commit()
                return result
            except Exception:
                self._discard_pooled_connection(rebuilt)
                raise
