...
``PostgresGatewayStore`` 既是 :class:`~lingxi.adapters.postgres_conversation._transaction._Transaction`
的唯一事务入口，也承载未开通首聊交接对账与队列失败提示的独立发送权。
"""
...
class PostgresGatewayStore:
    ...
    def __init__(
        ...
        *,
        ...
        """记下 DSN 与超时配置（新式 ``timeouts`` 或旧式毫秒参数二选一）。"""
        self._dsn = dsn
    ...
    def release_onboarding_claim(
        ...
        """把**自己那一次**认领放回 ``NULL``；是 :meth:`claim_stale_onboarding` 唯一的反向路径。
        ...
        **条件是 CAS，不是 ``IS NOT NULL``。** 只按事件标识清空会有 ABA：A 释放 → B 重新
        ...
        本身就是认领代次，拿它比对即可，不需要新列。``claim_token`` 为 ``None`` 时
        **什么都不做**：调用方没有认领代次，宁可留着不放，也不能撤销别人的认领。
        """
