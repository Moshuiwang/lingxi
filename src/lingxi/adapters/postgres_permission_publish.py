...
class LateOnboardingCandidate:
    ...
    ``already_ready`` 标出一种罕见但必须处理的中途状态，见
    :meth:`PostgresPermissionPublishStore.late_onboarding_recovery_candidates` 的文档。
    """
...
class PostgresPermissionPublishStore:
    ...
    def late_onboarding_recovery_candidates(
        ...
        这是 V-开通-18 恢复路径的每轮 tick 输入（见
        :mod:`lingxi.apps.scheduler.late_readiness_recovery`）。验收矩阵 V-开通-18 断言
        要求周期性回看那些停在 ``mcp_syncing``、已经超时的用户，重新确认就绪；一旦就绪
        就写 ``active`` 并主动通知——这条查询回答"现在该看谁"。

        ``reason`` 是必填的（与 :meth:`published_awaiting_readiness` 同一条纪律）：
        调用方必须显式说出自己在找哪一类意图，不给默认值——默认值会在新增一种
        ``reason`` 时静默地把恢复语义套用到不该套用的意图上（例如每日刷新排出的意图，
        那条链**没有** ``mcp_syncing`` 这个中间态可以"卡住"，误配上这条查询也不该悄悄
        生效）。

        五条筛选缺一不可：

        1. ``u.provisioning_state = 'mcp_syncing'``——只有还没被任何人推进到 ``active``
           的人才需要恢复；``account_state = 'enabled'`` 与
           :meth:`~lingxi.adapters.postgres_identity.PostgresAppUserStore.
           advance_provisioning_state` 推进到 ``active`` 时的守卫同一条口径，账号已经
           停用的人不该在这里被真实探针打扰。
        2. ``o.status = 'published'`` 且 ``o.reason = %(reason)s``——只认当前这一版
           **已经发布读回一致**、且属于调用方负责的那一类意图；
           ``o.permission_version = u.permission_version`` 把它锁定到用户**当前**这
           一版，防止一个人有多版历史意图时取错版本。
        3. ``EXISTS ... result = 'timed_out'``——只有真的判过超时的人才进候选集。这既是
           "只在超时之后才恢复"这条产品要求的落点，也天然避开了与
           :class:`~lingxi.core.identity.onboarding_runner.AutoOnboardingRunner` 自己
           那次阻塞式确认的并发探针：那次确认跑完（无论多久）才可能留下一条
           ``timed_out``，本查询因此不会在原始确认还在进行时把同一个人再捞出来探
           一次。
        4. ``progress.last_started_at`` 到期判据——**节奏由调用方给的
           ``recovery_interval_seconds`` 决定**，本方法只负责"只取到期的那些"，理由与
           :meth:`published_awaiting_readiness` 的同一条筛选相同：不去重的话，
           ``LIMIT`` 窗口会被一批"还没到期"的候选占满。
        5. ``o.payload ? 'permissions'``——payload 过了九十天会被
           :meth:`redact_expired_payloads` 擦成 ``'{}'``，擦过的快照说不出"当时发布
           的范围是什么"，据它渲染「开通完成」只会渲染出一句错话。

        **``already_ready``**：``bool_or(c.result = 'ready')`` 标出"上一轮已经探到
        就绪、但还没被推进 + 通知"的候选（真实发生场景：调用方在记完 ``ready`` 判定
        之后、推进 ``provisioning_state`` 之前崩溃）。这些候选**仍然满足上面的全部
        筛选**（``mcp_syncing`` 没被推进、也确实有过一条 ``timed_out``），调用方据此
        跳过再探一次，直接续做剩下的"推进 + 通知"两步——不这样处理的话，一次进程崩溃
        会让一个已经确认成功的人再也等不到「开通完成」，而这恰恰是本 Story 要修的那类
        缺口。

        **本方法跨读 ``mcp_sync_check``，是一处知情的例外**（与
        :meth:`published_awaiting_readiness` 同一条已登记的理由）：两张表同属"权限
        发布与就绪确认"这一个领域，本方法**只读**，写仍然只有
        ``postgres_mcp_token.record_attempt`` 一处。
        """
        ...
        with connect(self._dsn, timeouts=self._timeouts) as connection, connection.cursor() as cursor:
            cursor.execute(
                ...
                SELECT u.id, u.permission_version, o.payload ->> 'permissions',
                       progress.attempt_count, progress.has_ready
                  FROM app_user u
                  ...
                  JOIN LATERAL (
                         SELECT count(*) AS attempt_count,
                                ...
                                bool_or(c.result = 'ready') AS has_ready
                           FROM mcp_sync_check c
