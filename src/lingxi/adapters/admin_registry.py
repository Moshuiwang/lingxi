...
class PostgresAdminQueries:
    ...
    def resolve_identifier(self, *, identifier: str) -> str:
        ...
        零命中或多命中都是"反查失败"，与 ``resolve_identifier`` 的既有契约一致：
        原样返回输入，交给下游按既有"未找到"语义处理。

        迁移 ``0085``（rc25 S-2a）给 ``app_user`` 的规范化邮箱加了部分唯一索引，
        因此在跑过全链迁移的库上"多命中"结构性不可能。**多命中分支仍然保留**：
        它对 0085 之前建成的库、以及任何索引被删掉的场合仍然是唯一的防线，而
        本方法（管理员按邮箱找人）猜错一条的后果是把管理动作落到另一个人身上。
