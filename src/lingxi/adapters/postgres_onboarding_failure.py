"""``onboarding_failure`` 表（迁移 ``0077``）的唯一 PostgreSQL 落点。

写入方 :class:`PostgresFailureReasonRecorder`（两个写出点共用同一个实现，见
``lingxi.core.identity.onboarding_ports.FailureReasonRecorder`` 协议文档）与
只读查询 :func:`fetch_failure_reason`（供 ``/admin trace`` 消费）。

**为什么写入方是单条语句 INSERT，不是加入某个更大的事务**：两个审计写出点
本身只是结构化日志调用，紧邻它们的调用栈里没有一个已经打开、可供加入的数据库
事务——这是在现有写入模型下能做到的最强原子性，紧邻既有审计调用，不是异步、
不是"最终会补写"的旁路。
"""
...
class PostgresFailureReasonRecorder:
    """``FailureReasonRecorder`` 协议的真实实现。

    ``INSERT ... ON CONFLICT (trace_id) DO NOTHING``——``trace_id`` 是主键，
    同一条链正常只产生一次终态；若同一个 ``trace_id`` 被处理第二次（例如进程
    重启后重跑），先落的那一行保持不变，不覆盖、不报错。
    """
    ...
    def record_failure(self, *, trace_id: str, failure_reason: str, event_type: str) -> None:
        """按 ``trace_id`` 落一行失败原因；已存在同一 ``trace_id`` 时静默跳过。"""
...
def fetch_failure_reason(
    ...
    查无返回 ``None``（"这条链没有失败记录"，不是"查询失败"——两者由调用方
    分别处理，本函数不用异常表达"没找到"）。
    """
