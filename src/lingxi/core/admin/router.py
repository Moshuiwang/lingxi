...
**审计覆盖每一种结论**，含"未登记/已撤销/零角色"的拒绝——产品合同「管理员处理入口与
...
class AdminCommandRouter:
    ...
    def route(self, *, open_id: str, text: str, trace_id: str) -> AdminRouteOutcome:
        ...
        **每次调用都发起一次新的登记表读取**（通过注入的 ``registry``），不持有、
        ...
        if not is_authorized_admin(entry):
            # 未登记 / 已撤销 / 已登记但零角色，默认拒绝且**不区分**这三种原因——
            # 与产品合同"查询不到目标用户时返回明确的不存在结果，不猜测"同一姿态，
            # 不给探测者提供"你快登记成功了"一类的可利用信号。
            self._audit.record(
                "admin.command.rejected", reason="not_authorized", trace_id=trace_id
            )
            return AdminRouteOutcome(handled=False)
    ...
    def _dispatch(
        ...
        if command.kind is AdminCommandKind.HELP:
            self._audit.record(
                "admin.command.help", actor=entry.feishu_open_id, roles=roles, trace_id=trace_id
            )
            return AdminRouteOutcome(
                handled=True,
        ...
        self._audit.record(
            "admin.command.unknown", actor=entry.feishu_open_id, roles=roles, trace_id=trace_id
        )
        return AdminRouteOutcome(
            handled=True,
