...
class AdminCommandRouter:
    ...
    def _safe_record(self, action: str, /, **fields: object) -> bool:
        ...
        审计器是外部注入的协作对象，它的异常不能变成一次未处理的崩溃从 :meth:`route`
        逃出去——那样调用方拿到的既不是"拒绝"也不是"放行"，破坏的是"判定或执行失败均
        失败关闭"这条更基础的承诺。

        失败时改用标准库日志留一条安全记录（不经过可能已经损坏的审计器），只带动作名与
        异常类型，不带任何业务取值——它的定位是"审计器坏的那段时间发生过什么类型的
        交互"，不是替代真正的审计记录。
