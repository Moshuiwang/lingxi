...
class OutputConstraintResult:
    ...
    ``withheld`` 单独标记"整段拒发"这一类——这是本模块自身的从紧约定（模块文档
    「出口约束的产品合同」第 2 条，#141/#142/#149 产品负责人拍板），要求它有独立、
    可查询的判定，不是产品合同与外部边界正文的规定（2026-08-19 归属核对更正，
    见 Issue #238）。不能靠"blocked=True 且 text 恰好等于某个字符串"这种脆弱的
    隐式约定去猜。
    """
