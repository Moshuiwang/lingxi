...
# 正常静默留出余量；② ``apps/worker/turn.py`` 流式循环里工具返回
# （``tool_result``）时新增一次进度信号，把身份切到 composing（见
# ``apps/worker/service.py`` 的 ``on_stream_event``），让上面例子里 t=29
# 工具返回的那一刻就有一次新身份出现、停滞计时随之清零，不需要一直等到模型
# 真正开始输出文字。两个常量因此**不再取相同的值**（此前的版本要求两者相等，
