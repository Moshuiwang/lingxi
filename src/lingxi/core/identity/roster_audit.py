...
**口径变更登记（2026-08-08，产品负责人 D2 裁定）**：上面两段的「零新表」与
「只展示脱敏摘要」都**已被覆盖**——花名册读取需要一份跨重启的持久快照（表与迁移见
``0063_roster_snapshot``、``adapters/postgres_roster_snapshot.py``），受控管理群的日报
也**允许**展示姓名 / 工号 / 邮箱原值。本模块的比对语义与输出形态在 D2 之后**没有改**：
基线仍是建档存档三字段，:class:`PersonDiff` 仍不带值——取值属于渲染层的选择，比对层
不带值只是分层，不再是脱敏承诺的落点。渲染层按 D2 改造属 Issue #52 的后续 Story，
不得据本模块现状声称日报已按 D2 交付。
