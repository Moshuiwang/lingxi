...
class OnboardingSteps:
    ...
    def _match(
        ...
        if aggregate.granted and not self._publish_allowed():
            # **翻译层不可用：一条发布意图都不排**，授权与撤权都不例外。停在这里而不是
            # 继续，因为另外两种说法都不诚实：不是"没有银河权限"（他明明有，那样说会把他
            # 引去申请一个已经有的权限），也不是"同步超时"（那说的是外部慢，而这里是我们
            # 自己缺一份内容）；更不能"先建档建环境、发布那步以后再补"：
            # - 也不能"先建档建环境、发布那步以后再补"：合同要求成功以发布 + 就绪确认
            #   为前提，半开的用户会一直停在同步中而没有任何人会来收拾。
            # 因此按本侧故障收口，且**在建档之前**，不留下任何半成品。这道闸只保护"银河
            # 内容需要翻译才能安全发布"这件事——零银河的人没有银河内容要翻译，本地授权
            # 本身已经是精确指标名，结构上从不到达这里。
    ...
    def _record_decision(self, user_id: str, row: Any, *, trace_id: str) -> Any:
        ...
        落决定要求账号有效：复核账号状态与这一步之间隔着令牌签发与用户环境创建，管理员
        恰好在这个窗口停用账号是真实形状，只有落决定那把行锁里的复检能真正把它串起来。

        被拦下时**必须收敛到既有的停用终态，不能变成通用内部故障**：用户看到的仍是「你的
        账号当前已停用」。审计动作与另外两处复核逐字相同——运维按同一个动作名检索"这条链
        因为账号状态停下了"，不需要区分是哪一次复核抓到的。事务整体回滚：版本没推进、
        意图没入队。
