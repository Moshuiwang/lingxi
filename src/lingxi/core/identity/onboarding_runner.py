...
class AutoOnboardingRunner(OnboardingSteps):
    ...
    def _notify_admin_of_failure(self, *, reason: str, event_id: str, trace_id: str) -> None:
        ...
        三条路径共用它，不允许出现第四条漏网的：停机中拒绝开新链、提交执行器失败，以及
        执行线程自己算出的失败终态。前两条是同步返回，此前根本不经过执行线程，于是用户
        看到「已转交管理员处理」而管理群什么都没收到——文案承诺与实际行为对不上。

        同步超时也走这里：合同对十五分钟超时的措辞同样是"转交管理员处理"。原因码沿用各自
        终态的取值，管理员据此能分清"这是同步在等"还是"这是本侧真的坏了"。这条告警只是
        "让管理群知道"，自动恢复仍由迟到就绪恢复职责独立完成，两者不互相依赖。
        """
    ...
    def _notify(
        ...
        *,
        ...
        """主动私聊一条消息。失败只留响亮审计，不改写终态。

        系统触发（预开通，没有入站消息）**不发消息、按送达处理**。
    ...
    def _abort_if_stalled(self, user_id: str | None, terminal: _Terminal, *, trace_id: str) -> None:
        """已经确定失败终态、且这个人早前被推进到"开通中"时，**当场**收口。
    ...
    def _run(
        ...
        *,
        ...
        Args:
            event_id: 触发这条链的事件标识；系统触发时是一个合成值。
            open_id: 这个人的外部标识。
            trace_id: 追溯号。
            stalled: 调用方传入的可变容器——这个人一旦被推进到"开通中"就把用户标识写进去；
                从那一刻起本链任何失败终态都可能让他停在中途格，调用方需要这个信号才能
                决定要不要当场收口。
            grant: 预开通带来的预授权；首聊路径恒为 ``None``。
    ...
    def _identify(self, open_id: str, *, trace_id: str) -> _Terminal | tuple[Any, Any]:
        ...
        同邮箱那道闸挡在**建档之前**——早于任何数据库写入、早于令牌签发与采纳（采纳会按
        邮箱把正式表里**别人的**密文拿过来）、早于存量差集导入、也早于任何发布意图。判据
        是外部标识，因为建档之前本人还没有内部用户标识。

        放在公共段而不是某条入口分支里：这条链会被"收到首聊消息"与"预开通"共同复用，挂在
        分支上的闸对第二条路径等于不存在。
