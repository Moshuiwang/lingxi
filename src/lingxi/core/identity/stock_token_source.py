...
首聊或预开通建档后，按邮箱查一次旧系统写入的正式表。本端口只如实返回旧行形状；
旧行没有人员标识或工号，不能独立证明历史归属，因此开通链只在明确查无旧行时继续
签新，存在任何旧行都沿身份未决路径关闭。真正的飞书读取与 AES 解密在
`adapters/stock_token_bitable.py`。四态仍分开表达：正式表能不能查到这一行、这一行有没有密文，与密文解不
解得开，是两件事、两种责任主体——前者是外部数据的形状，后者是我们这边的主密钥
配不配得上。混成一种"三态"会让"解密失败"这种响亮失败被调用方一次 ``if/else``
悄悄归并成"没有可用密文"，从而退回签新——签新会让这个人的用户环境令牌与正式表
错位，造成真实 MCP 认证失败。因此 :data:`ADOPTABLE`（源端拿到可用明文）与
...
NO_ROW = "no_row"
#: 正式表有这一行，但 ``token_cipher`` 列是空的：仍是归属不明的旧行，不签新覆盖。
NO_CIPHER = "no_cipher"
#: 正式表有这一行、有密文，且已用受控主密钥解密成功：:attr:`StockTokenLookup.secret`
#: 是旧系统留下的明文候选；开通链在没有独立历史归属证据时不采纳。
ADOPTABLE = "adoptable"
