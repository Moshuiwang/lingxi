...
class ConversationRecord:
    ...
    #: ``running_task_id`` 指向的那条 ``task`` 当前的 ``status``（忙碌提示文案如实
    #: 区分"排队中"与"处理中"）。``running_task_id`` 为 ``None`` 时恒为 ``None``；
    #: 否则应为 ``'queued'``/``'running'`` 等 `task.status` 取值之一，与
...
class OnboardingResult:
    ...
    ``grant_not_applied``（只供批量清单口径）：系统触发（预开通）本轮带着名单
    预授权、却因为用户**已经 active** 在续行前复核处提前收口时为真——终态仍是
    ``completed``（这个人确实开通着，不是失败），但名单答应的那笔授权**没有**
    落库。ops 清单据此单独归类，不计入"成功预开通"；是否给已 active 用户补上
    名单权限是产品语义，这里绝不静默扩权。首聊路径（无预授权）恒为 ``False``，
    gateway 不读它。
    """
...
#: 释放、交给下一轮重新捞。与「跑过了、得到了一个失败结论」是两件完全不同的事——后者
#: 不释放（重跑不会改变结论，只会持续冲击外部系统）。住在 ``ports`` 而不是编排实现里：
#: 它是 :class:`OnboardingRunner` 合同的一部分，认领方要照它分流。
...
class GatewayTransaction(Protocol):
    ...
    **同一事务**里，因此这些方法刻意只在事务对象上提供，拿不到"事务外顺手写一条"的入口。
    ...
    def discard_stale_agent_session(self, *, conversation_id: str) -> None:
        """入队判定「不续用旧会话」时，把已判废的 ``agent_session_id`` 置空并排队物理清理。

        与入队同事务；与 ``clear_agent_session``（/new）不同：调用时本事务已抢占
        话题，不做忙碌判定，也不触碰已送达正文的保留边界。
        """
    ...
    def peek_preprovision_notice(self, *, user_id: str) -> PendingPreprovisionNotice | None:
        ...
        **不消费**一次性标志——与 :meth:`consume_preprovision_notice` 拆成两步：渲染可能失败（权限快照过了
        保留期、内容目录与代码分两次合入的中间态），先消费再渲染会把一次性标志白白
        烧掉、这个人永远收不到那句话；因此必须先拿快照、渲染成功后才消费。并发安全
        不靠本方法：两个事务同时 peek 到同一句挂起，最终只有一个能在 consume 的
        原子 ``UPDATE`` 上命中。``permissions`` 是当前权限版本已发布的文档文本，
        快照不可用时为 ``None``，由调用方按渲染失败处理。
        """
    ...
    def consume_preprovision_notice(self, *, user_id: str) -> bool:
        ...
        ``UPDATE``，与调用方所在的入站消息事务一起提交或回滚。**按用户**而不是
        按话题：预开通是给这个人开的，他第一次说话落在哪个话题上都算首聊。调用
        次序见 :meth:`peek_preprovision_notice`：渲染成功之后才允许消费。
        """

    def list_user_memory(self, *, user_id: str) -> list[UserMemoryEntry]:
        """按用户取全部记忆，``/memory list`` 用。"""
    ...
    def remember_user_memory(
        ...
        新增触达上限时返回 ``None`` 且不写入，不做静默截断。
        """

    def forget_user_memory(self, *, user_id: str, memory_id: str) -> UserMemoryEntry | None:
        """删除属于该用户的一条记忆，返回被删除那一行的内容。

        调用方据此在回执里回显，供用户自校验删对了；未删除任何行时返回 ``None``。
...
class GatewayStore(Protocol):
    ...
    def release_onboarding_claim(
        ...
        :meth:`claim_stale_onboarding` 是「取出即记账」，而在它之后这条事件仍然可能
        根本没被执行（执行器满位、停机信号刚好落在中间、同一个人已有链在跑）——没有
        这条反向路径，那些事件此后永远不会再被任何人认领。**只放回真的没跑成的**：
        已得出结论的事件不释放，重跑不会改变结论，只会持续冲击外部系统。
        ``claim_token`` 是**认领代次**，实现必须拿它做 CAS（只有行上的值仍等于这个
        值时才清空，否则有 ABA 风险）；``None`` 表示调用方没有认领代次，实现必须
        什么都不做。
        """
...
class VersionResolver(Protocol):
    ...
    后不再改变。分流规则形态未定，默认实现固定返回 ``stable``；开成注入口是为了
    让 `V-灰度-02` 能在测试里造出两个版本的任务。
    """

    def __call__(self, *, user_id: str, now: datetime) -> str:
        """按用户与请求时刻求出这次任务的目标 worker 版本。"""
        ...
