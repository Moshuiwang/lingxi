...
class OnboardingReplyRenderer:
    ...
    def render_result(
        ...
        「正在核对，请稍候」就是这一轮的完整交代。其余状态渲染为空时（包括说不出
        范围的 ``completed``）一律落到内部故障终态——「已转交管理员处理」这句话在
        这里是准确的：编排返回了一个渲染不出来的结果，确实需要有人看一眼。

        Args:
            result: 编排返回的结论。
            checking_key: 「正在核对」的文案键，由 gateway 自己发过一次，这里跳过。
            message: 触发这次开通的入站消息，提供追溯号与审计字段。
    ...
    def render_preprovision_notice(
        ...
        *,
        ...
        公司/职能取值与开通链宣告成功时**同一来源、同一姿势**：该用户当前权限版本
        已发布的权限文档，经同一个范围描述函数说成两串展示文本。

        **不能因为一句附加提示打断这个人的问数**——他这条消息的正常处理与这句话无关，
        因此两类失败都只留审计、返回 ``None``；调用方据此**不消费**一次性标志，下一条
        消息还会再试（「只提示一次」保持，「失败即永远丢失」消除）。

        Returns:
            渲染好的那一句；文案侧失败或权限快照不可用时返回 ``None``。
        """
        try:
