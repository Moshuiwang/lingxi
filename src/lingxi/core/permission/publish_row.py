...
``None``。理由是改写者与旧系统的合法密文在那两条路径上**不可区分**，而"存量用户令牌
归属"是待产品负责人裁定的产品结（[#203](https://github.com/Moshuiwang/lingxi/issues/203)
已登记）；猜错方向会把一行合法的旧记录打成永久失败。这两条路径由**就绪探针**兜底——
带着错误密文的行永远探不成功，以十五分钟超时转运维暴露，不产生用户可见的假成功。
