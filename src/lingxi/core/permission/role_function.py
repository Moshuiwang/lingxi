...
本模块只认配置：映射唯一来源是仓库内的配置文件，产品负责人在仓库里持续维护。
配置未覆盖的角色名保留原文并标记「未映射」，不做"去掉开头的 A"这类猜测——银河
角色命名规则不统一，含临时角色与个人专用角色，猜测会造出错误的职能标签，而
错误的范围比没有范围更糟。未映射角色不产生 Lingxi 支持职能，没有任何映射成功
的角色时用户按未授权处理。映射结果只用于展示与授权资格判断，不改动落库的原始
角色名，也不自行扩大实际权限——实际可查询范围仍由 MCP 按已同步权限决定。
"""
