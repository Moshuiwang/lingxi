...
| :data:`SHAPE_FULL_WILDCARD` | ``"*"`` 键的值恰为 ``["*"]`` | 一组 ``company_id="*"`` × 映射全部指标（:func:`all_metrics`） |
| :data:`SHAPE_ALL_SCOPE_EXPLICIT` | ``"*"`` 键的值是不含 ``"*"`` 的显式指标列表 | 一组 ``company_id="*"`` × 该列表 |
...
#: 「全部」组在管理卡上显示的职位标签；同时是识别该组的判据（``company_id="*"``
#: 且 ``position_name`` 恰为此值）。
...
def missing_all_scope_metrics(
    ...
    """对当前生效的「全部」组（``direction=grant``、``company_id="*"``、
    ``position_name`` 为 :data:`ALL_SCOPE_POSITION_NAME`、带组 ID），算出映射里已有、
    组里还没有的指标——新指标进来时重算据此补行。只看传入的（生效）条目：撤销过的组
    不在其中，因此不会复活。返回 ``{组 ID: (缺的指标, …)}``，没有缺项的组不出现。"""
