...
窗口内**没有任何任务**（两个计数都是 0）是一个合法的确定结论（"这一天没有
任务"），不是不可判定，与本模块其余段落对"空窗口"的既有处理方式一致。
...
#: 结构性地没有可用的 token 用量数字（迁移 0070 之前产生、或从未真正跑过一次
#: 回合），因此没有任何数字可以聚合。
RESOURCE_USAGE_ALL_NULL_REASON = (
    "本窗口内的任务在 task.token_usage 字段上全部为 NULL（可能产生于迁移 0070 落地"
    "之前，或从未真正进入过执行回合、结构性地没有可落库的用量数字），没有任何数字"
    "可以聚合"
)
#: 同上，对应 ``task.guard_denied_count``。
DENIED_COUNT_ALL_NULL_REASON = (
    "本窗口内的任务在 task.guard_denied_count 字段上全部为 NULL（原因与 token 用量"
    "一致，见 RESOURCE_USAGE_ALL_NULL_REASON），没有任何数字可以聚合"
)
