...
def _render_local_override_activity(
    ...
    **措辞如实化（Trace #328 opus 审查；2026-08-29 再次如实化，PM 裁定，Issue
    #419）**：正文用「本窗口」不用「今日」——本段与本报告其余六段统计的是同一个
    UTC 窗口，而这个窗口实际是「昨天」（`[D-1, D)`，模块文档「投递结果段为什么用
    一个独立、更早的窗口」一节旁的既有约定），把它说成「今日」并不准确。正文用
    「生效」不用「登记」——四源合并此前挂在 `aggregate.granted` 判据**之后**，
    一个当前没有任何银河权限的用户走的是撤权分支，从不到达本地覆盖合并那一步，
    「当前生效」这句话对这类用户当时不成立（#319 动机场景，`V-权限-15` 曾登记的
    已知限制），因此改用更保守的「登记」——只断言"这一行确实写进了本地覆盖表"，
    不断言它此刻是否真的影响了这个人的最终权限。**该限制已随 PM 2026-08-29 裁定
    消除**：零银河权限用户的本地授权现在无条件参与合并（下一轮重算或下一次开通
    链会把它发布出去），"登记 ≠ 生效"这条保留措辞的理由不再成立，改回「生效」——
    与本段其余计数（本窗口新增授权/抑制/收回笔数）一样，是对当前真实权限状态的
    断言，不再是留了一手的弱化说法。
    """
