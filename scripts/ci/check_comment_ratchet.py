...
- ``provenance``：本文件的注释（含行尾注释）与 docstring 正文里，命中"来历"
  ...
  覆盖 Issue/PR 编号简写（``#238``）、日期（``2026-09-04``）、``Issue``、
  ``PR #123``、``审查``/``审核``/``修复包``/``codex``/``Codex``/``裁定``/
  ``复盘``/``复核``——这些词是"这行在交代来历，不是在交代约束"的信号（代码
  框架「三、横切约定」：注释与 docstring 说明"为什么"，不留审查过程/修复包/
  日期这类会腐烂的痕迹）。
...
与文件体量棘轮的一处刻意不同：文件被删除时，``check_size_ratchet.py`` 的登记
条目静默放行（``measure()`` 里那个文件已经"测不到"）；这里 ``current`` 只保留
非零计数，一个 (类别, 路径) 从 ``current`` 里消失——无论是文件被删还是计数真的
清零到 0——都等价于"实测 0"，与基线记录的非零值不一致，同样需要显式
``--refresh`` 才能清空登记，不允许陈旧登记静默残留。
"""
