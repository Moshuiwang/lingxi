...
#   并在这里带理由改这个数。
#
#   2026-09-07 抬到 75471B（+1010B，Trace #606）：新增的不是沿革也不是实现细节，
...
#   零余量规则照旧：抬到的就是实测值，下一次增长仍要在这里自证。**这段措辞本身
#   尚待产品负责人在 PR 中确认**（Trace #606 决策清单 Z-5 仍是待裁定项）。
PER_FILE_BUDGET_BYTES = {
    "docs/当前能力.md": 54 * 1024,
    "docs/产品合同与外部边界.md": 75471,
}
