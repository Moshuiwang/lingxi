...
#
# 2026-09-13（Issue #748，Trace #770 批次一）上调到 43KB：「验证与门禁」新增第三档门禁
# `Epic Verdict` 的正式口径（裁决层的存在位置与身份、路径 a / b 的判定、两步漂移残余、
# 恢复路径），+1.8KB，是当前有效的门禁事实而不是沿革。调整前三文件合计 40843B，距
# 40KB 上限只剩 117B。新上限按调整后实测 42701B 留约 1.3KB 余量，与上一次上调同一
# 口径：**刻意不留大空间**，下一次扩容仍须自证理由。
TOTAL_BUDGET_BYTES = 43 * 1024
