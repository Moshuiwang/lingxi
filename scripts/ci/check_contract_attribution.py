...
GROUNDED_ATTRIBUTIONS: tuple[GroundedAttribution, ...] = (
    ...
    GroundedAttribution(
        # Issue #479 分册：花名册审计日报一节搬到审计与保留分册，正文逐字节未变。
        "docs/技术设计/验收矩阵-审计与保留.md",
    ...
    GroundedAttribution(
        ...
        # 2026-08-29 维护批（Issue #413）为符合开工必读体量预算瘦身措辞（去掉
        # 完整 Issue 链接与「归属核对」四字，语义不变），登记表同步逐字更新。
        "docs/技术设计/代码框架.md",
    ...
    # 原「合同明确排除，此处固化为"不存在的接口"而非"会拒绝的接口"：」一行随
    # 接口设计第六节 2026-08-29 精简（Issue #413，产品负责人 2026-08-24 拍板：
    # 管理 MCP 精简为未来入口指路，不再保留工具清单正文）一并删除，登记项同步移除。
    GroundedAttribution(
        "docs/技术设计/接口设计.md",
        "`authorized` 为 `true` 才代表交付完成。授权未确认时返回 `delivered: false` 与说明，由 Agent 按合同措辞告知用户核对，**不自动重发**。",
        "交付规则",
    ),
    # 原「约定：enqueue_publish 与 audit.record……这是合同要求，不能靠代码评审保证。」
    # 一行随 Trace #643 IN-13（2026-09-06 编排者裁定）改写为按接口族分述的准确表述
    # 一并删除，登记项同步移除：改写后的句子不再包含"合同"字样，不再命中本门禁的
    # 触发字面，见接口设计.md「八、领域服务接口」现行「约定」段与 8.2/8.3 小节。
    GroundedAttribution(
        "docs/技术设计/数据库设计.md",
    ...
    # 原「高级工作台」登记项（"合同要求'高级工作台中产生的正式产物通过统一的交付
    # Skill 完成交付'……"）随 JumpServer 高级工作台 2026-08-28 取消而退场：架构设计
    # 5.5 节改写为不再引用已废止的合同章节（Trace #373 S-H2-6），新文本不含
    # TRIGGER_PATTERN 会命中的归属短语，无需替换登记。
    GroundedAttribution(
        "docs/技术设计/架构设计.md",
    ...
    GroundedAttribution(
        # Issue #479 分册：任务队列与入队一节搬到接入与管理分册，正文逐字节未变。
        "docs/技术设计/验收矩阵-接入与管理.md",
    ...
    GroundedAttribution(
        # #493 的职位+公司范围授权设计逐字对照了合同中的本地权限覆盖与管理员入口条款。
        "docs/技术设计/职位与公司范围授权.md",
