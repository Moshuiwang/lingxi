...
# 部署动作。**关键动作与语义参照 `deploy/README.md`**（迁移、profile 选择、
# 健康检查两段判定、持久卷、回滚机制均照该文档），但不是逐行照抄——例如
# healthcheck 的 `--role` 取值按服务映射而不是与 service 名相同（`worker-queue`
# 容器用 `--role worker`，因为 `apps/healthcheck` 目前没有为它单独注册角色，
# 见下面 `HEALTHCHECK_ROLE_BY_SERVICE`），这类差异在对应步骤函数里注明依据，
# 不假装每一行都是原文复制。
#
...
# healthcheck 的 `--role` 取值：`apps/healthcheck/__init__.py` 的
# `_DSN_ENV_VAR_BY_ROLE` 只登记了 scheduler/worker/gateway 三个键，没有
# `worker-queue`——`worker-queue` 与一次性 `worker` job 共用同一个镜像、同一套
# `LINGXI_POSTGRES_DSN`，因此 `deploy/compose.yaml` 里 worker-queue 服务的
# healthcheck 用的是 `--role worker`（2026-08-18 编排者修复包 F1：此前本脚本
# 对 worker-queue 直接传 `--role worker-queue`，argparse `choices` 校验会拒绝，
# 会在真实 Stage 上当场失败）。
declare -A HEALTHCHECK_ROLE_BY_SERVICE=(
  [scheduler]="scheduler"
  [gateway]="gateway"
  [worker-queue]="worker"
)
...
step_description() {
  case "$1" in
    preflight) echo "部署前置检查：七个 env 文件权限 0600、镜像 tag 已设置、受控触发夹具三面（宿主 shell/env 文件/容器）均干净" ;;
    migrate) echo "跑一次性迁移作业（job profile run --rm migrate）" ;;
    up) echo "以 mvp profile 启动 scheduler + gateway + 常驻 queue worker" ;;
    healthcheck) echo "三个常驻服务的健康回读（docker exec 语义，不开放端口；worker-queue 用 --role worker）" ;;
    ...
    rollback-drill) echo "切到指定的上一个候选镜像 tag（--rollback-tag 必填）并重新 up -d，核对镜像确实切换且旧候选能在当前库结构上正常启动" ;;
    cleanup) echo "down、清理临时资源、受控触发夹具三面均确认已清空" ;;
    *) echo "未知步骤" ;;
  esac
}
...
用法：
  scripts/stage_rehearsal_mvp_acceptance.sh --self-check
      ...
      并跑一次受控触发夹具的只读清空检查（宿主 shell 这一面）。在任何机器上
      都可以安全执行，包括本仓库以外的开发机——这是实施者应该使用的唯一模式。

  scripts/stage_rehearsal_mvp_acceptance.sh --list-steps
      列出全部步骤、一句话说明与预计耗时，不执行任何步骤。

  scripts/stage_rehearsal_mvp_acceptance.sh --step <id> [--confirm-real-run] [--rollback-tag <tag>]
      单独执行（或演练）某一步。不带 --confirm-real-run 时只打印将要执行的
      命令，不实际运行。--confirm-real-run 只应在 biai-stage 上、且当前
      目录已按 deploy/README.md 准备好七个 env 文件时使用。执行 rollback-drill
      步骤时必须同时提供 --rollback-tag，否则拒绝执行（避免空转仍判通过）。

  scripts/stage_rehearsal_mvp_acceptance.sh --all --confirm-real-run --rollback-tag <tag>
...
# ---------------------------------------------------------------------------
# 受控触发夹具三面检查（2026-08-18 编排者修复包 F2/P1-F）：夹具经 env 文件注入
# 容器，不是注入执行本脚本的宿主 shell，因此"用后撤除并回读"必须三面都查：
# 1) 宿主 shell 进程环境；2) 七个 env 文件；3) 实际运行中容器的 env 输出。
# preflight（开窗前）与 cleanup（收窗后）都调用同一个函数，判据不各写一遍。
# ---------------------------------------------------------------------------
check_fixtures_three_sides() {
  local failures=0
  ...
  log "  夹具检查 2/3：七个 env 文件"
  if python3 "${repository_root}/scripts/acceptance_fixtures.py" --check-files "${ENV_FILES[@]}"; then
    log "    通过"
  else
    failures=$((failures + 1))
  fi

  log "  夹具检查 3/3：运行中容器的实际环境（gateway、worker-queue——这两个容器分别装配"
  log "    LINGXI_GATEWAY_CARD_FAILURE_INJECT 与 LINGXI_WORKER_OUTPUT_SAFETY_CANARY）"
  if [[ "${CONFIRM_REAL_RUN}" == "1" ]]; then
    local service
    for service in gateway worker-queue; do
      if docker compose --env-file "${ENV_FILE}" -f "${COMPOSE_BASE}" -f "${COMPOSE_STAGE_OVERLAY}" \
           exec -T "${service}" env | python3 "${repository_root}/scripts/acceptance_fixtures.py" --check-files -; then
        log "    ${service}：通过"
      else
        log "    ${service}：失败"
        failures=$((failures + 1))
    ...
    printf '（dry-run）将对 gateway、worker-queue 两个容器分别执行：\n'
    printf '  docker compose ... exec -T <service> env | python3 scripts/acceptance_fixtures.py --check-files -\n'
  fi
...
self_check() {
  ...
  log "自检 4/5：受控触发夹具当前宿主 shell 进程环境是否干净（只读检查，不修改任何东西；"
  log "  这只是三面检查中的一面——env 文件与容器内环境只能在 biai-stage 上用 --confirm-real-run 核实）"
  if python3 "${repository_root}/scripts/acceptance_fixtures.py" --check-clear; then
    log "  通过"
  else
    log "  失败：当前宿主 shell 环境里残留受控触发夹具变量，须先清除再继续"
    failures=$((failures + 1))
  fi
...
# ---------------------------------------------------------------------------
# 各步骤实现。关键动作参照 deploy/README.md（见文件头说明），本脚本负责排序、
# dry-run 包装、fixture 检查与失败提前退出（set -e 已保证）。
# ---------------------------------------------------------------------------
...
step_preflight() {
  ...
  log "preflight：受控触发夹具三面检查（开窗前必须三面均干净，验收窗口专用夹具由执行卡④阶段显式打开）"
  if ! check_fixtures_three_sides; then
    log "  失败：至少一面查到残留，须先清除再继续（不回显具体取值）"
    return 1
  fi
}
...
step_healthcheck() {
  ...
  local service role
  for service in scheduler gateway worker-queue; do
    role="${HEALTHCHECK_ROLE_BY_SERVICE[${service}]}"
    run_or_print docker compose --env-file "${ENV_FILE}" \
      -f "${COMPOSE_BASE}" -f "${COMPOSE_STAGE_OVERLAY}" \
      exec -T "${service}" python -m lingxi.apps.healthcheck --role "${role}"
  done
...
step_persistence_restart() {
  ...
  log "  合规的结果**有两种**，不是只有一种（V-部署-12 原文）：在途任务或者收口为唯一"
  log "  stopped 终态，或者仍保持 running、等待未来一次心跳超时被回收——两者都不判失败；"
  log "  真正的失败信号是任务被记成两次终态、或结果丢失、或重复交付"
  run_or_print docker compose --env-file "${ENV_FILE}" \
    -f "${COMPOSE_BASE}" -f "${COMPOSE_STAGE_OVERLAY}" \
...
本步骤**不构成**#62 线「备份恢复」的完整证据——只是环境核对，真实备份/恢复
动作与结论仍需另行登记（见员工旅程验收检查单 F 组）。
NOTE
...
step_rollback_drill() {
  if [[ -z "${ROLLBACK_TARGET_TAG}" ]]; then
    ...
    log "  理由（P2-6 修复）：不给这一步一个「看起来跑完了但其实没真的切换镜像」"
    log "  就判通过的机会——那样会被 ④-2 #62 线误当成 V-部署-05 的真实证据"
    return 1
  fi
  log "rollback-drill：目标候选 tag = ${ROLLBACK_TARGET_TAG}（V-部署-05 的真实环境复演）"
...
step_cleanup() {
  ...
  if [[ "${CONFIRM_REAL_RUN}" == "1" ]]; then
    log "  夹具检查 1/3：宿主 shell 进程环境"
    python3 "${repository_root}/scripts/acceptance_fixtures.py" --check-clear || true
    log "  夹具检查 2/3：七个 env 文件"
    python3 "${repository_root}/scripts/acceptance_fixtures.py" --check-files "${ENV_FILES[@]}" || true
    log "  夹具检查 3/3：容器已 down，跳过（无运行中容器可查）"
  else
    printf '（dry-run）将检查宿主 shell 与七个 env 文件是否已清空受控触发夹具变量\n'
  fi
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --rollback-tag) ROLLBACK_TARGET_TAG="${2:-}"; shift 2 ;;
    -h|--help) usage; exit 0 ;;
    *)
      echo "未知参数：$1" >&2
      usage
