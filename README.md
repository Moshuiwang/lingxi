...
/admin grant_permission <open_id> <公司ID> <指标名> <原因>   本地补授（确认卡）
/admin suppress_permission <open_id> <公司ID> <指标名> <原因> 本地抑制（确认卡）
...
- 补授/抑制生效时点：当前随每日权限重算（UTC 零点/进程重启）批量发布；「确认后即时生效」改进中，见 issue。
