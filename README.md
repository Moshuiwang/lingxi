...
仓库文档是长期产品事实的唯一正文；动态工作统一管理：Issue 记录需求、缺陷、研究与待决策事项，PR 记录实现和验证，具体 Trace 的优先级、负责人和进度记录在 [docs/traces/](docs/traces/README.md)（对应 `[tracking]` Issue 只作瘦指针）。关闭 Issue 前，仍有效的结论必须回写到仓库文档。
