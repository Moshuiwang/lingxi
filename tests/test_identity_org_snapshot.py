...
def batch(
    ...
    return SnapshotBatch(
        ...
        departments=(
            SnapshotDepartment(tenant_key="tenant_a", department_key="dept_a", name="测试部门"),
        ),
        members=members,
    )
...
class BatchIntegrityTest(unittest.TestCase):
    def test_a_member_row_outside_both_visible_paths_is_incomplete(self) -> None:
        ...
        bad = SnapshotBatch(
            tenants=(
                TenantScope("tenant_a", True, frozenset({"ou_zhang"}), frozenset({"ou_zhang"})),
            ),
    ...
    def test_paths_that_see_different_member_sets_fail_the_batch(self) -> None:
        ...
        report = verify_batch(
            batch((member(),), app_keys=frozenset({"ou_member_one", "ou_only_visible_to_app"}))
        )
    ...
    def test_a_duplicated_open_id_fails_the_batch(self) -> None:
        ...
        report = verify_batch(
            batch(
                (
                    member(),
                    member(member_key="ou_dup_key", user_id="user_two", union_id="union_two"),
                )
            )
        )
    ...
    def test_matching_department_sets_on_both_paths_do_not_fail_the_batch(self) -> None:
        ...
        report = verify_batch(
            SnapshotBatch(
                ...
                departments=(
                    SnapshotDepartment(
                        tenant_key="tenant_a", department_key="dept_a", name="测试部门"
                    ),
                ),
                members=(member(),),
            )
    ...
    def test_a_tenant_invisible_to_the_user_identity_fails_the_batch(self) -> None:
        ...
        scope = TenantScope(
            "tenant_a", False, frozenset({"ou_member_one"}), frozenset({"ou_member_one"})
        )
        report = verify_batch(SnapshotBatch(tenants=(scope,), departments=(), members=(member(),)))
    ...
    def test_a_member_that_belongs_to_no_declared_tenant_fails_the_batch(self) -> None:
        stray = member(
            tenant_key="tenant_unknown",
            member_key="ou_stray",
            open_id="ou_stray",
            user_id="user_stray",
            union_id="union_stray",
        )
        ...
        report = verify_batch(
            SnapshotBatch(
                ...
                members=(*original.members, stray),
            )
        )
    ...
    def test_a_member_key_seen_by_the_path_but_missing_a_row_fails_the_batch(self) -> None:
        ...
        scope = TenantScope(
            "tenant_a",
            True,
            frozenset({"ou_member_one", "ou_counted_only"}),
            frozenset({"ou_member_one", "ou_counted_only"}),
        )
        report = verify_batch(SnapshotBatch(tenants=(scope,), departments=(), members=(member(),)))
...
class SnapshotExpiryTest(unittest.TestCase):
    ...
    def test_a_batch_expires_ninety_days_after_it_started(self) -> None:
        ...
        self.assertEqual(
            snapshot_expires_at(self.started_at), self.started_at + timedelta(hours=2160)
        )
    ...
    def test_availability_turns_stale_exactly_at_expiry_and_unavailable_without_a_batch(
        self,
    ) -> None:
        expires_at = snapshot_expires_at(self.started_at)

        self.assertIs(
            directory_availability(expires_at, expires_at - timedelta(seconds=1)),
            DirectoryAvailability.AVAILABLE,
        )
        self.assertIs(directory_availability(expires_at, expires_at), DirectoryAvailability.STALE)
        self.assertIs(
            directory_availability(expires_at, expires_at + timedelta(days=1)),
            DirectoryAvailability.STALE,
        )
        self.assertIs(
            directory_availability(None, self.started_at), DirectoryAvailability.UNAVAILABLE
        )
